...
function addSection(pageId) {
  ...
  document.getElementById("modalBody").innerHTML = `
    ...
    <div class="form-row"><label>正文内容</label><textarea class="form-control" name="content" rows="5"></textarea></div>
    <div class="form-row"><label>排序</label><input type="number" class="form-control" name="sort_order" value="0"></div>`;
  document.getElementById("modalSave").onclick = async () => {
    const data = {
      page_id: pageId,
      subtitle: document.querySelector('#modalBody [name="subtitle"]').value,
      content: document.querySelector('#modalBody [name="content"]').value,
...
function editSection(sectionId) {
  ...
  api("GET", `/api/page-section/${sectionId}`).then(s => {
    ...
    document.getElementById("modalBody").innerHTML = `
      ...
      <div class="form-row"><label>正文内容</label><textarea class="form-control" name="content" rows="5">${escapeHtml(s.content)}</textarea></div>
